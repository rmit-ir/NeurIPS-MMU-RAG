...
from typing import List, Dict, Any, NamedTuple, TypedDict
...
def run_search_for_topic(topic: EvaluateRequest, num_docs: int) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        docs = [r._asdict() for r in search_results if isinstance(r, SearchResult)]
        output_record = OutputRecord(iid=topic.iid, query=topic.query, docs=docs)

        logger.info("Search completed successfully", topic_id=topic.iid, docs_count=len(docs))
        return output_record
        
    except Exception as e:
        logger.error("Error processing topic", topic_id=topic.iid, error=str(e))
        return {
            'iid': topic.iid,
            'query': topic.query,
...
def save_results(results: List[OutputRecord], output_file: str):
    ...
    try:
        ...
        logger.info("Results saved successfully", output_path=output_path, results_count=len(results))

    except Exception as e:
        logger.error("Error saving results", output_path=output_file, error=str(e))
        sys.exit(1)
...
Examples:
  ...
  python scripts/run_retrieval.py --topics-file data/past_topics/processed/topics.rag24.test.n50.jsonl --num-docs 100
    ...
    logger.info("Topics loaded successfully", topics_count=len(topics), topics_file=args.topics_file)

    # Process topics
    results = []
    for i, topic in enumerate(topics, 1):
        logger.info("Processing topic", topic_id=topic.iid, progress=f"{i}/{len(topics)}")
    ...
    logger.info("Processing completed successfully", results_count=len(results))
